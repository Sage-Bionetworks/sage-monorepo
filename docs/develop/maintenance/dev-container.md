...
| `.aws/config`                             | Developer AWS SSO profiles. `AWS_CONFIG_FILE` points here instead of the default `~/.aws/config`, so they survive a rebuild.                                             | Git- and Docker-ignored. Persist `config` only, never credentials or the SSO token cache.                                                                  |
...
| AWS profile not found | Profiles written to `~/.aws/config`, not `$AWS_CONFIG_FILE` | Move them to `$AWS_CONFIG_FILE` | Check with `aws configure list-profiles`; SSO tokens still need `aws sso login` |
