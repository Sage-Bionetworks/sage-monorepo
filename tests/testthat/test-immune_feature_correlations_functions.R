test_that("Build Immune Feature Correlations Value Table", {
    response_tbl <- dplyr::tibble(sample_id = 1:3, feature_id = 1L, value = 1:3)
    feature_tbl  <- dplyr::tribble(
        ~sample_id, ~feature_id, ~value, ~order, ~feature,
        1,          1,           .1,     2,      "f1",
        1,          2,           .2,     1,      "f2",
        1,          3,           .3,     3,      "f3",
        2,          1,           .4,     2,      "f1",
    ...
    sample_tbl <- dplyr::tibble(sample_id = 1:3, group = rep("C1", 3))
