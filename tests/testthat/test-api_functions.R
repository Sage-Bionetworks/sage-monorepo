with_test_api_env({
    ...
    test_that("perform_api_query", {
        ...
        result <- perform_api_query("test_query1", lst, ghql_query_object)
        expect_named(result, "tags")
        expect_error(
            perform_api_query("test_query2", lst, ghql_query_object),
            "Query: test_query2 not in avaialble queries: test_query1"
        )
    })
})
