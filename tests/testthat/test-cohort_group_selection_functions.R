# TODO: uncomment TCGA test when cohort selection is faster
# tcga_samples = "TCGA" %>%
...
test_that("Build Tag Cohort Object", {
  ...
  res2 <- build_tag_cohort_object("PCAWG", pcawg_samples, "Immune_Subtype")
  ...
  # expect_named(res1, expected_object_names)
  expect_named(res2, expected_object_names)
  ...
  expect_equal(res2$group_name, "Immune_Subtype")
  expect_equal(res2$group_type, "tag")
})
...
feature_bin_sample_tbl <- build_feature_bin_sample_tbl(
  "PCAWG", pcawg_samples, "B_cells_Aggregate2", 2L
)
...
test_that("Build Feature Bin Group Tibble", {
  res1 <- build_feature_bin_group_tbl(
    feature_bin_sample_tbl, "B_cells_Aggregate2"
  )
...
test_that("Build Feature Bin Cohort Object",{
  res1 <- build_feature_bin_cohort_object(
    "PCAWG", pcawg_samples, "B_cells_Aggregate2", 2L, feature_bin_tbl
  )
  expect_named(res1$sample_tbl, c("sample", "group"))
  expect_named(
    res1$group_tbl,
