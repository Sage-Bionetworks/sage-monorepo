...
export class ModelOrgLogoService {
  /**
   ...
   */
  getLogoUrl(name: string | null | undefined): string | null {
   ...
   */
  isMonoLogo(name: string | null | undefined): boolean {
    const slug = this.slugify(name);
    return slug !== null && ORG_LOGO_VARIANT[slug] === 'mono';
  }

  /**
   * Resolve a free-text org name to a canonical logo slug. Returns `null`
   * when the name is empty or has no recognizable match.
   *
   ...
   */
  slugify(name: string | null | undefined): string | null {
    if (!name) return null;
