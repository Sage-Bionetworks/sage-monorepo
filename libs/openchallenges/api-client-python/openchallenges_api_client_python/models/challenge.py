"""OpenChallenges API Challenge model (auto-generated + small patch).

NOTE: This file is generated by OpenAPI Generator. A minimal custom patch was
added in `from_dict` to tolerate empty `platform` objects returned by the API.
Future regenerations should re-apply that logic if still needed.
"""
...
class Challenge(BaseModel):
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        # CUSTOM PATCH: tolerate empty platform object (all fields None) by treating as None
        platform_raw = obj.get("platform")
        platform_val = None
        if isinstance(platform_raw, dict):
            # consider it empty if all expected keys are missing or None
            meaningful = [platform_raw.get(k) for k in ("id", "slug", "name")]
            if any(v is not None for v in meaningful):
                try:
                    platform_val = SimpleChallengePlatform.from_dict(platform_raw)
                except Exception:
                    platform_val = None

        input_data_raw = obj.get("inputDataTypes")
        if input_data_raw is not None:
            input_data_val = [EdamConcept.from_dict(_item) for _item in input_data_raw]
        else:
            input_data_val = None

        operation_raw = obj.get("operation")
        if operation_raw is not None:
            try:
                operation_val = EdamConcept.from_dict(operation_raw)
            except Exception:
                operation_val = None
        else:
            operation_val = None
