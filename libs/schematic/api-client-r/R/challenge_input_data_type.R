#' Create a new ChallengeInputDataType
#'
#' @description
...
#' @description ChallengeInputDataType Class
...
#' @export
ChallengeInputDataType <- R6::R6Class(
  ...
  public = list(
    ...
    #' To JSON String
    #'
    #' @return ChallengeInputDataType in JSON format
    #' @export
    toJSON = function() {
      ChallengeInputDataTypeObject <- list()
      if (!is.null(self$`id`)) {
        ChallengeInputDataTypeObject[["id"]] <-
          self$`id`
      }
      if (!is.null(self$`slug`)) {
        ChallengeInputDataTypeObject[["slug"]] <-
          self$`slug`
      }
      if (!is.null(self$`name`)) {
        ChallengeInputDataTypeObject[["name"]] <-
          self$`name`
      }
      if (!is.null(self$`createdAt`)) {
        ChallengeInputDataTypeObject[["createdAt"]] <-
          self$`createdAt`
      }
    ...
    #' @return the instance of ChallengeInputDataType
    #' @export
    ...
    #'
    #' @description
    #' Validate JSON input with respect to ChallengeInputDataType and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`id`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ChallengeInputDataType: the required field `id` is missing."))
      }
      ...
      if (!is.null(input_json$`slug`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ChallengeInputDataType: the required field `slug` is missing."))
      }
      ...
      if (!is.null(input_json$`name`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ChallengeInputDataType: the required field `name` is missing."))
      }
      ...
      if (!is.null(input_json$`createdAt`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ChallengeInputDataType: the required field `createdAt` is missing."))
      }
      ...
      if (!is.null(input_json$`updatedAt`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ChallengeInputDataType: the required field `updatedAt` is missing."))
      }
    },
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of ChallengeInputDataType
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# ChallengeInputDataType$unlock()
#
## Below is an example to define the print function
# ChallengeInputDataType$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ChallengeInputDataType$lock()
