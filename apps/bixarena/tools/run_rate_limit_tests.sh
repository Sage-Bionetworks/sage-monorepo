...
run_interactive() {
    ...
    echo ""
    uv run locust -f bixarena_tools/load_testing/rate_limit_test.py --host="$HOST"
}
...
run_general_test() {
    ...
    echo ""
    uv run locust -f bixarena_tools/load_testing/rate_limit_test.py \
        --host="$HOST" \
        --headless \
        -u 5 \
        -r 1 \
...
main() {
    ...
    if [ ! -f "bixarena_tools/load_testing/rate_limit_test.py" ]; then
