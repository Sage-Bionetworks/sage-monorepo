...
class RateLimitTestUser(HttpUser):
    ...
    def _handle_response(self, response, expected_limit: int):
        ...
        if response.status_code == 429:
            ...
            if not rate_limit:
                response.failure("429 response missing X-RateLimit-Limit header")
                return

            if not retry_after:
                response.failure("429 response missing Retry-After header")
                return
            ...
            if not rate_limit:
                response.failure("Success response missing X-RateLimit-Limit header")
                return
...
class RateLimitBurstTestUser(HttpUser):
    """
    User class for burst testing - rapidly hits a single endpoint to
    trigger rate limits.
...
def on_test_stop(environment, **kwargs):
    ...
    if hasattr(environment, "burst_test_results"):
        ...
        if total_requests > 0:
            ...
            print(
                f"  ✅ Successful:        {successful_requests:>6} "
                f"({success_percentage:>5.1f}%)"
            )
            print(
                f"  🚫 Rate Limited:      {rate_limited_requests:>6} "
                f"({rate_limit_percentage:>5.1f}%)"
            )

            if rate_limited_requests > 0:
                print(
                    f"\n✓ Rate limiting is working! "
                    f"{rate_limited_requests} requests were "
