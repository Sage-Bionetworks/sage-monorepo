...
ALTER TABLE api.battle
  ADD COLUMN example_prompt_id UUID NULL
  REFERENCES api.example_prompt(id) ON DELETE SET NULL;

-- Partial index: only non-null FKs participate in trending queries.
CREATE INDEX idx_api_battle_example_prompt_id
  ON api.battle(example_prompt_id)
  WHERE example_prompt_id IS NOT NULL;
...
UPDATE api.battle b
SET example_prompt_id = sub.ep_id
FROM (
