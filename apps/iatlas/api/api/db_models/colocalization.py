...
class Colocalization(Base):
    ...
    coloc_dataset_id = db.Column(
        db.String, db.ForeignKey("datasets.id"), nullable=False
    )

    feature_id = db.Column(db.String, db.ForeignKey("features.id"), nullable=False)
    ...
    data_set = db.relationship(
        "Dataset",
        backref=orm.backref("colocalizations_primary", uselist=True, lazy="noload"),
        uselist=False,
        lazy="noload",
        primaryjoin="Dataset.id==Colocalization.dataset_id",
    )

    coloc_data_set = db.relationship(
        "Dataset",
        backref=orm.backref("colocalizations_secondary", uselist=True, lazy="noload"),
        uselist=False,
        lazy="noload",
        primaryjoin="Dataset.id==Colocalization.coloc_dataset_id",
    )

    feature = db.relationship(
        "Feature",
        backref=orm.backref("colocalizations", uselist=True, lazy="noload"),
        uselist=False,
        lazy="noload",
    )
