...
class Node(Base):
    ...
    node_feature_id = db.Column(db.String, db.ForeignKey("features.id"), nullable=True)

    node_gene_id = db.Column(db.String, db.ForeignKey("genes.id"), nullable=True)

    tag_1_id = db.Column(db.String, db.ForeignKey("tags.id"), nullable=False)

    tag_2_id = db.Column(db.String, db.ForeignKey("tags.id"), nullable=True)

    data_set = db.relationship(
        "Dataset",
        backref=orm.backref("node", uselist=True, lazy="noload"),
        uselist=False,
        lazy="noload",
    )
