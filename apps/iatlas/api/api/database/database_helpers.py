...
def build_general_query(
    model, args=[], accepted_option_args=[], accepted_query_args=[]
):
    option_args = build_option_args(*args, accepted_args=accepted_option_args)
    query_args = build_query_args(model, *args, accepted_args=accepted_query_args)
...
def _create_table_as(element, compiler, **kw):
    text = element.query.statement.compile(
        dialect=postgresql.dialect(), compile_kwargs={"literal_binds": True}
    )
    query = "CREATE TEMP TABLE %s AS %s" % (element.name, text)
    return query
