...
def build_driver_result_request(
    ...
    feature_requested,
    mutation_requested,
    mutation_gene_requested,
    mutation_type_requested,
    tag_requested,
    data_set=None,
    distinct=False,
    entrez=None,
    feature=None,
    max_p_value=None,
    max_log10_p_value=None,
    min_fold_change=None,
    min_log10_fold_change=None,
    min_log10_p_value=None,
    min_p_value=None,
    min_n_mut=None,
    min_n_wt=None,
    mutation=None,
    ...
    from .mutation import (
        get_mutation_column_labels,
        get_mutation_type_column_labels,
    ...
    core |= get_mutation_column_labels(mutation_requested, mutation_1)
    core |= get_simple_gene_column_labels(mutation_gene_requested, gene_1)
    core |= get_mutation_type_column_labels(mutation_type_requested, mutation_type_1)
    ...
    if (max_log10_p_value or max_log10_p_value == 0) and (
        not max_p_value and max_p_value != 0
    ):
        query = query.filter(driver_result_1.log10_p_value <= max_log10_p_value)

    if min_fold_change or min_fold_change == 0:
        query = query.filter(driver_result_1.fold_change >= min_fold_change)

    if (min_log10_fold_change or min_log10_fold_change == 0) and (
        not min_fold_change and min_fold_change != 0
    ):
        query = query.filter(driver_result_1.log10_fold_change >= min_log10_fold_change)

    if (min_log10_p_value or min_log10_p_value == 0) and (
        not min_p_value and min_p_value != 0
    ):
        query = query.filter(driver_result_1.log10_p_value >= min_log10_p_value)
    ...
    if "dataSet" in requested or data_set or related:
        ...
        data_set_join_condition = build_join_condition(
            data_set_1.id,
            driver_result_1.dataset_id,
            filter_column=data_set_1.name,
            filter_list=data_set,
        )
        query = query.join(data_set_1, and_(*data_set_join_condition), isouter=is_outer)

    if "feature" in requested or feature:
        is_outer = not bool(feature)
        data_set_join_condition = build_join_condition(
            feature_1.id,
            driver_result_1.feature_id,
            filter_column=feature_1.name,
            filter_list=feature,
        )
        query = query.join(feature_1, and_(*data_set_join_condition), isouter=is_outer)
    ...
    if "tag" in requested or tag:
        ...
        query = query.join(tag_1, and_(*data_set_join_condition), isouter=is_outer)
    ...
    if "mutation" in requested or mutation:
        ...
        mutation_join_condition = build_join_condition(
            driver_result_1.mutation_id,
            mutation_1.id,
            filter_column=mutation_1.name,
            filter_list=mutation,
        )
        query = query.join(mutation_1, and_(*mutation_join_condition), isouter=is_outer)
    ...
    return get_pagination_queries(
        query, paging, distinct, cursor_field=driver_result_1.id
    )
