...
def fetch_page(query, paging, distinct):
    ...
    res = query.limit(limit + 1).all()
    return res


def process_page(
    items, count_query, paging, distinct, response_builder, pagination_requested
):
    ...
    if paging_type == Paging.OFFSET or distinct == True:
        ...
        results = deque(map(response_builder, items) if response_builder else items)
        pageInfo["startCursor"] = (
            to_cursor_hash(results[0]["id"]) if (len(results) > 0) else None
        )
        pageInfo["endCursor"] = (
            to_cursor_hash(results[-1]["id"]) if (len(results) > 0) else None
        )
    ...
    return {"items": results, "paging": pageInfo}


def paginate(
    query, count_query, paging, distinct, response_builder, pagination_requested
):
