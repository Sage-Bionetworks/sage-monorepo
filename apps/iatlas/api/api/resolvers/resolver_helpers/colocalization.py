...
def build_coloc_graphql_response(colocalization):
    return {
        ...
        "colocDataSet": build_data_set_graphql_response(prefix="coloc_data_set_")(
            colocalization
        ),
...
def build_colocalization_request(
    ...
    core |= get_selected(coloc_data_set_requested, coloc_data_set_core_field_mapping)
    ...
    if "colocDataSet" in requested or coloc_data_set:
        ...
        data_set_join_condition = build_join_condition(
            coloc_data_set_1.id,
            colocalization_1.coloc_dataset_id,
            filter_column=coloc_data_set_1.name,
            filter_list=coloc_data_set,
        )
        query = query.join(
            coloc_data_set_1, and_(*data_set_join_condition), isouter=is_outer
        )
    ...
    if "snp" in requested or snp:
        ...
        query = query.join(snp_1, and_(*snp_join_condition), isouter=is_outer)

    return get_pagination_queries(
        query, paging, distinct, cursor_field=colocalization_1.id
    )
