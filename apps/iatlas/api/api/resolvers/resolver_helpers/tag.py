...
tag_request_fields = simple_tag_request_fields.union(
    {"publications", "related", "sampleCount", "samples"}
)
...
def has_tag_fields(item, prefix="tag_"):
    if not item:
        return False
    return (
        get_value(item, prefix + "id")
        or get_value(item, prefix + "name")
        or get_value(item, prefix + "characteristics")
        or get_value(item, prefix + "short_display")
        or get_value(item, prefix + "long_display")
        or get_value(item, prefix + "type")
        or get_value(item, prefix + "order")
    )


def build_tag_graphql_response(
    requested=[],
    sample_requested=[],
    ...
    def f(tag):
        ...
        publication_dict = get_publications(
            tag_id=tag_id,
            requested=requested,
            publications_requested=publications_requested,
        )
        ...
        result = {
            ...
            "sampleCount": (
                len(sample_dict) if sample_dict and "sampleCount" in requested else None
            ),
            "publications": (
                map(build_publication_graphql_response, publication_dict)
                if publication_dict
                else None
            ),
            "related": (
                map(
                    build_tag_graphql_response(requested=related_requested),
                    related_dict,
                )
                if related_dict
                else None
            ...
            "samples": (
                map(build_sample_graphql_response(), sample_dict)
                if sample_dict and "samples" in requested
                else None
            ),
        }
        return result
...
def build_tag_request(
    ...
    if data_set:
        ...
        dataset_join_condition = build_join_condition(
            dataset_to_tag_1.dataset_id,
            dataset_1.id,
            filter_column=dataset_1.name,
            filter_list=data_set,
        )
        dataset_subquery = dataset_subquery.join(
            dataset_1, and_(*dataset_join_condition), isouter=False
        )
    ...
    if related:
        ...
        related_join_condition = build_join_condition(
            tag_to_tag_1.related_tag_id,
            tag_1.id,
            filter_column=tag_1.name,
            filter_list=related,
        )
        related_subquery = related_subquery.join(
            tag_1, and_(*related_join_condition), isouter=False
        )
    ...
    if sample:
        ...
        sample_join_condition = build_join_condition(
            sample_to_tag_1.sample_id,
            sample_1.id,
            filter_column=sample_1.name,
            filter_list=sample,
        )
        sample_subquery = sample_subquery.join(
...
def get_publications(tag_id, requested, publications_requested):
    if "publications" in requested:
        ...
        core |= {pub_1.id.label("id"), tag_to_pub_1.tag_id.label("tag_id")}
        ...
        pub_query = pub_query.join(tag_to_pub_1, and_(*tag_tag_join_condition))
...
def get_related(tag_id, requested, related_requested):
    if "related" in requested:
        ...
        tag_sub_query = tag_sub_query.join(tag_1, and_(*tag_tag_join_condition))

        related_query = related_query.filter(related_tag_1.id.in_(tag_sub_query))
...
def get_samples(tag_id, requested, sample_requested, cohort=None, sample=None):
    if "samples" in requested or "sampleCount" in requested:
        ...
        tag_subquery = sess.query(sample_to_tag_1.sample_id).filter(
            sample_to_tag_1.tag_id.in_([tag_id])
        )

        sample_query = sample_query.filter(sample_1.id.in_(tag_subquery))
