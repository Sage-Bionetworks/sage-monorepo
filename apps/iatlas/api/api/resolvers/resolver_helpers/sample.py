...
feature_related_sample_request_fields = simple_sample_request_fields.union({"value"})
...
mutation_related_sample_request_fields = sample_request_fields.union({"status"})
    ...
    def f(sample):
        if not sample:
            ...
            dict = {
                ...
                "tag": (
                    build_tag_graphql_response()(sample)
                    if has_tag_fields(sample)
                    else None
                ),
            }
            return dict
...
def build_gene_expression_graphql_response(prefix="sample_"):
    ...
    def f(sample):
        if not sample:
            ...
            result_dict["nanostringExpr"] = get_value(
                sample, prefix + "gene_nanostring_expr"
            )
            return result_dict
...
def build_sample_mutation_join_condition(
    sample_to_mutation_model,
    sample_model,
    mutation_status,
    mutation_id=None,
    status=None,
    ...
    join_condition = build_join_condition(
        sample_to_mutation_model.sample_id,
        sample_model.id,
        filter_column=sample_to_mutation_model.mutation_id,
        filter_list=mutation_id,
    )
    if mutation_status:
        join_condition.append(sample_to_mutation_model.status == mutation_status)
...
def build_sample_request(
    ...
    core_field_mapping = {"name": sample_1.name.label("sample_name")}
    ...
    if data_set:
        ...
        data_set_sub_query = (
            sess.query(data_set_1.id).filter(data_set_1.name.in_(data_set))
            if data_set
            else data_set
        )
    ...
    if feature or feature_class:
        ...
        query = query.join(
            feature_to_sample_1, feature_to_sample_1.sample_id == sample_1.id
        )
        ...
        if feature_class:
            feature_class_join_condition = build_join_condition(
                feature_class_1.id,
                feature_1.class_id,
                feature_class_1.name,
                feature_class,
            )
            query = query.join(feature_class_1, and_(*feature_class_join_condition))
