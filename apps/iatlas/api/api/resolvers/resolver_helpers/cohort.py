...
def build_cohort_graphql_response(
    requested=[],
    sample_requested=[],
    sample_tag_requested=[],
    feature_requested=[],
    gene_requested=[],
    mutation_requested=[],
    ...
    def f(cohort):
        if not cohort:
            ...
            samples = get_samples(
                cohort_id, requested, sample_requested, sample_tag_requested
            )
...
def get_samples(id, requested, sample_requested, tag_requested):
    if "samples" not in requested:
        ...
        if "tag" in sample_requested:
            ...
            query = query.join(tag_1, and_(*sample_tag_join_condition), isouter=True)
...
def get_features(id, requested, feature_requested):
    if "features" not in requested:
        ...
        query = query.join(feature_1, and_(*feature_join_condition), isouter=False)
...
def get_genes(id, requested, gene_requested):
    if "genes" not in requested:
        ...
        gene_join_condition = build_join_condition(cohort_to_gene_1.gene_id, gene_1.id)

        query = query.join(gene_1, and_(*gene_join_condition), isouter=False)
...
def get_mutations(id, requested, mutation_requested, mutation_gene_requested):
    ...
    if "mutations" not in requested:
        ...
        core |= get_selected(mutation_gene_requested, mutation_gene_core_field_mapping)
        ...
        query = query.join(mutation_1, and_(*mutation_join_condition), isouter=False)
        ...
        if "gene" in mutation_requested:
            ...
            query = query.join(
                gene_1, and_(*mutation_gene_join_condition), isouter=False
            )
