...
from .sample import (
    ...
    build_gene_expression_graphql_response,
    build_single_cell_seq_response,
)
...
def build_gene_graphql_response(
    ...
    def f(gene):
        ...
        result_dict = {
            ...
            or get_value(gene, prefix + "entrez_id"),
            "hgnc": get_value(gene, prefix + "hgnc")
            or get_value(gene, prefix + "hgnc_id"),
            ...
            "samples": map(build_gene_expression_graphql_response(), samples),
            "pseudoBulkSamples": map(
                build_single_cell_seq_response(), pseudobulk_samples
            ),
...
def build_pub_gene_gene_type_join_condition(
    gene_ids, gene_type, pub_gene_gene_type_model, pub_model
):
    join_condition = build_join_condition(
        pub_gene_gene_type_model.publication_id,
        pub_model.id,
        pub_gene_gene_type_model.gene_id,
        gene_ids,
    )
...
def build_gene_request(
    ...
    cell_to_gene_1 = aliased(CellToGene, name="celltogene")
    ...
    if max_rna_seq_expr or min_rna_seq_expr or sample:
        ...
        if sample:
            ...
            sample_join_condition = build_join_condition(
                gene_to_sample_1.sample_id,
                sample_1.id,
                filter_column=sample_1.name,
                filter_list=sample,
            )
            gene_to_sample_subquery = gene_to_sample_subquery.join(
    ...
    if cohort:
        ...
        cohort_subquery1 = sess.query(pseudobulk_1.gene_id)
        ...
        cohort_subquery2 = sess.query(cohort_to_gene_1.gene_id)

        cohort_join_condition2 = build_join_condition(
            cohort_to_gene_1.cohort_id,
            cohort_1.id,
            filter_column=cohort_1.name,
            filter_list=cohort,
        )
        ...
        gene_id_subquery3 = sess.query(cell_to_gene_1.gene_id)
        gene_id_subquery3 = gene_id_subquery3.filter(
            cell_to_gene_1.cell_id.in_(cell_id_subquery)
        )

        query = query.filter(
            gene_1.id.in_(cohort_subquery1)
            | gene_1.id.in_(cohort_subquery2)
            | gene_1.id.in_(gene_id_subquery3)
        )
...
def get_samples(
    ...
    core_field_mapping = {
        ...
        "rnaSeqExpr": gene_to_sample_1.rna_seq_expression.label(
            "sample_gene_rna_seq_expr"
        ),
        "nanostringExpr": gene_to_sample_1.nanostring_expression.label(
            "sample_gene_nanostring_expr"
        ),
    }
    ...
    if max_rna_seq_expr:
        query = query.filter(gene_to_sample_1.rna_seq_expression <= max_rna_seq_expr)
    ...
    if cohort:
        ...
        query = query.filter(sample_1.id.in_(cohort_subquery))

    samples = query.distinct().all()
    return samples


def get_cell_type_samples(
    gene_id, requested, cell_type_sample_requested, cohort=None, sample=None
):
    ...
    if "cellType" in cell_type_sample_requested:
        sample_core |= {pseudobulk_1.cell_type.label("sample_cell_type")}
    ...
    query = query.join(pseudobulk_1, and_(*sample_join_condition))
...
def get_gene_types(gene_id, requested, gene_types_requested, gene_type=None):
    ...
    gene_gene_type_join_condition = build_join_condition(
        gene_to_gene_type_1.gene_set_id,
        gene_type_1.id,
        gene_to_gene_type_1.gene_id,
        [gene_id],
    )

    if gene_type:
        gene_gene_type_join_condition.append(gene_type_1.name.in_(gene_type))

    gene_type_query = gene_type_query.join(
        gene_to_gene_type_1, and_(*gene_gene_type_join_condition)
    )
...
def get_publications(gene_id, requested, publications_requested):
    ...
    core_field_mapping = {
        ...
        "firstAuthorLastName": pub_1.first_author_last_name.label(
    ...
    ptgtgt_join_condition = build_join_condition(
        pub_1.id,
        pub_gene_gene_type_1.publication_id,
        filter_column=pub_gene_gene_type_1.gene_id,
        filter_list=[gene_id],
    )
    query = query.join(
        pub_gene_gene_type_1, and_(*ptgtgt_join_condition), isouter=False
    )
...
def get_pseudobulk_samples(
    gene_id, requested, sample_requested, cohort=None, sample=None
):
    ...
    if "singleCellSeqSum" in sample_requested:
        sample_core |= {
            pseudobulk_1.single_cell_seq_sum.label("sample_single_cell_seq_sum")
        }
    ...
    sample_join_condition = build_join_condition(
        pseudobulk_1.sample_id, sample_1.id, pseudobulk_1.gene_id, gene_id
    )
    query = query.join(pseudobulk_1, and_(*sample_join_condition))
...
def get_cells(gene_id, requested, cell_requested, cohort=None):
    ...
    if "singleCellSeq" in cell_requested:
        cell_core |= {cell_to_gene_1.single_cell_seq.label("cell_single_cell_seq")}
    ...
    cell_to_gene_join_condition = build_join_condition(
        cell_1.id, cell_to_gene_1.cell_id, cell_to_gene_1.gene_id, gene_id
    )
    query = query.join(cell_to_gene_1, and_(*cell_to_gene_join_condition))
    ...
    if cohort:
        ...
        gene_id_subquery = sess.query(cell_to_gene_1.gene_id)
        gene_id_subquery = gene_id_subquery.filter(
            cell_to_gene_1.cell_id.in_(cell_id_subquery)
        )

        query = query.filter(gene_1.id.in_(gene_id_subquery))
