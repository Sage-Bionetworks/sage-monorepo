...
def build_germline_gwas_result_request(
    ...
    if max_p_value or max_p_value == 0:
        query = query.filter(germline_gwas_result_1.p_value <= float(max_p_value))

    if min_p_value or min_p_value == 0:
        query = query.filter(germline_gwas_result_1.p_value >= float(min_p_value))

    if "dataSet" in requested or data_set:
        is_outer = not bool(data_set)
        data_set_join_condition = build_join_condition(
            data_set_1.id,
            germline_gwas_result_1.dataset_id,
    ...
    if "feature" in requested or feature:
        ...
        feature_join_condition = build_join_condition(
            feature_1.id,
            germline_gwas_result_1.feature_id,
    ...
    if "snp" in requested or snp:
        ...
        snp_join_condition = build_join_condition(
            snp_1.id,
            germline_gwas_result_1.snp_id,
            filter_column=snp_1.name,
            filter_list=snp,
        )
        query = query.join(snp_1, and_(*snp_join_condition), isouter=is_outer)

    return get_pagination_queries(
        query, paging, distinct, cursor_field=germline_gwas_result_1.id
    )
