...
def get_mutation_column_labels(requested, mutation, add_id=False):
    mapping = {
        "name": mutation.name.label("mutation_name"),
        "mutationCode": mutation.mutation_code.label("mutation_code"),
    }
...
def build_mutation_graphql_response(
    requested=[],
    sample_requested=[],
    status=None,
    sample=None,
...
def build_mutation_request(
    ...
    mutation_core = get_mutation_column_labels(requested, mutation_1, add_id=True)
    ...
    if sample:
        sample_subquery = sess.query(sample_to_mutation_1.mutation_id)

        sample_join_condition = build_join_condition(
            sample_to_mutation_1.sample_id,
            sample_1.id,
    ...
    if cohort:
        ...
        cohort_join_condition = build_join_condition(
            cohort_to_mutation_1.cohort_id,
            cohort_1.id,
...
def build_simple_mutation_request(
    query,
    requested,
    mutation_obj,
    gene_obj,
    ...
    All positional arguments are required. Positional arguments are:
        1st position - a sql query
        2nd position - a set of the requested fields at the root of the graphql request
        3rd position - a mutation table object
        4th position - a gene table object
        5th position - a mutation type table object
    All keyword arguments are optional. Keyword arguments are:
        `entrez` - a list of integers, gene entrez ids
        `mutation` - a list of strings, mutation names
        `mutation_code` - a list of strings, mutation codes
    ...
    if "gene" in requested or entrez:
        ...
        gene_join_condition = build_join_condition(
            gene_obj.id,
            mutation_obj.gene_id,
            filter_column=gene_obj.entrez_id,
            filter_list=entrez,
        )
        query = query.join(gene_obj, and_(*gene_join_condition), isouter=is_outer)
    ...
    if "mutationType" in requested or mutation_type:
        ...
        mutation_type_join_condition = build_join_condition(
            mutation_type_obj.id,
            mutation_obj.mutation_type_id,
            filter_column=mutation_type_obj.name,
            filter_list=mutation_type,
        )
        query = query.join(
            mutation_type_obj, and_(*mutation_type_join_condition), isouter=is_outer
        )

    if mutation_code:
        query = query.filter(mutation_obj.mutation_code.in_(mutation_code))

    return query


def get_samples(
    mutation_id, requested, sample_requested, status=None, sample=None, cohort=None
):
    ...
    core_field_mapping = {
        ...
        "status": sample_to_mutation_1.mutation_status.label("sample_mutation_status"),
    }
    ...
    if status:
        query = query.filter(sample_to_mutation_1.mutation_status.in_(status))
    ...
    if cohort:
        ...
        query = query.filter(sample_to_mutation_1.sample_id.in_(cohort_subquery))
