...
def build_copy_number_result_request(
    ...
    if (max_log10_p_value or max_log10_p_value == 0) and (
        ...
        query = query.filter(copy_number_result_1.log10_p_value <= max_log10_p_value)

    if (min_log10_p_value or min_log10_p_value == 0) and (
        not min_p_value and min_p_value != 0
    ):
        query = query.filter(copy_number_result_1.log10_p_value >= min_log10_p_value)
    ...
    if min_mean_normal or min_mean_normal == 0:
        query = query.filter(copy_number_result_1.mean_normal >= min_mean_normal)
    ...
    if data_set or "dataSet" in requested or related:
        ...
        data_set_join_condition = build_join_condition(
            data_set_1.id,
            copy_number_result_1.dataset_id,
    ...
    if entrez or "gene" in requested:
        ...
        data_set_join_condition = build_join_condition(
            gene_1.id,
            copy_number_result_1.gene_id,
            filter_column=gene_1.entrez_id,
            filter_list=entrez,
        )
        query = query.join(gene_1, and_(*data_set_join_condition), isouter=is_outer)

    if feature or "feature" in requested:
        is_outer = not bool(feature)
        data_set_join_condition = build_join_condition(
            feature_1.id,
            copy_number_result_1.feature_id,
    ...
    return get_pagination_queries(
        query, paging, distinct, cursor_field=copy_number_result_1.id
    )
