...
patient_request_fields = simple_patient_request_fields.union({"samples", "slides"})


def build_patient_graphql_response(
    requested=dict(), slide_requested=dict(), sample=None, slide=None, prefix="patient_"
):
...
def build_patient_request(
    ...
    if max_age_at_diagnosis:
        query = query.filter(patient_1.age_at_diagnosis <= max_age_at_diagnosis)

    if min_age_at_diagnosis:
        query = query.filter(patient_1.age_at_diagnosis >= min_age_at_diagnosis)
    ...
    if sample or data_set:
        ...
        query = query.join(sample_1, and_(*sample_join_condition), isouter=is_outer)
    ...
    if slide:
        slide_join_condition = build_join_condition(
            patient_1.id,
            slide_1.patient_id,
            filter_column=slide_1.name,
            filter_list=slide,
        )
        query = query.join(slide_1, and_(*slide_join_condition), isouter=False)
