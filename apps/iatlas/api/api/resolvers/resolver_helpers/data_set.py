...
def build_data_set_graphql_response(
    prefix="data_set_", requested=[], sample_requested=[], tag_requested=[], sample=None
):
...
def build_data_set_request(
    ...
    if sample:
        ...
        data_set_to_sample_subquery = sess.query(data_set_to_sample_1.dataset_id)
        ...
        data_set_to_sample_subquery = data_set_to_sample_subquery.join(
...
def get_samples(dataset_id, requested, sample_requested, sample=None):
    if "samples" in requested:
        ...
        data_set_to_sample_subquery = sess.query(data_set_to_sample_1.sample_id)

        data_set_to_sample_subquery = data_set_to_sample_subquery.filter(
            data_set_to_sample_1.dataset_id == dataset_id
        )

        sample_query = sample_query.filter(sample_1.id.in_(data_set_to_sample_subquery))
