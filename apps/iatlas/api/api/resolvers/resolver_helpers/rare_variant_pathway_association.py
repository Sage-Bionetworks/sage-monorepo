...
def build_rare_variant_pathway_association_request(
    ...
    if pathway:
        query = query.filter(rare_variant_pathway_association_1.pathway.in_(pathway))

    if max_p_value or max_p_value == 0:
        query = query.filter(rare_variant_pathway_association_1.p_value <= max_p_value)

    if min_p_value or min_p_value == 0:
        query = query.filter(rare_variant_pathway_association_1.p_value >= min_p_value)

    if "dataSet" in requested or data_set:
        is_outer = not bool(data_set)
        data_set_join_condition = build_join_condition(
            data_set_1.id,
            rare_variant_pathway_association_1.dataset_id,
    ...
    if "feature" in requested or feature:
        ...
        data_set_join_condition = build_join_condition(
            feature_1.id,
            rare_variant_pathway_association_1.feature_id,
    ...
    return get_pagination_queries(
        query, paging, distinct, cursor_field=rare_variant_pathway_association_1.id
    )
