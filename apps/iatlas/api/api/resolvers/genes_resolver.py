...
def resolve_genes(
    ...
    gene_types_requested = get_requested(
        ...
        requested_field_mapping=simple_gene_set_request_fields,
        child_node="geneTypes",
    )

    publications_requested = get_requested(
        selection_set=selection_set,
        requested_field_mapping=simple_publication_request_fields,
        child_node="publications",
    )

    samples_requested = get_requested(
        selection_set=selection_set,
        requested_field_mapping=gene_related_sample_request_fields,
        child_node="samples",
    )

    pseudobulk_sample_requested = get_requested(
        selection_set=selection_set,
        requested_field_mapping=cell_type_gene_related_sample_request_fields,
        child_node="pseudoBulkSamples",
    )

    cell_requested = get_requested(
        selection_set=selection_set,
        requested_field_mapping=gene_related_cell_request_fields,
        child_node="cells",
    )
