...
def test_build_general_query(db_session):
    ...
    test_1 = build_general_query(
        model,
        args=[query_arg_1, query_arg_2, option_value_1],
        accepted_option_args=accepted_option_args,
        accepted_query_args=accepted_query_args,
    )
    test_2 = build_general_query(
        model,
        args=[query_arg_1, query_arg_2],
        accepted_option_args=accepted_option_args,
        accepted_query_args=accepted_query_args,
    )
    ...
    assert str(test_1.statement.compile(dialect=postgresql.dialect())) == str(
        db_session.query(model)
        .options(orm.joinedload(option_value_1))
        .statement.compile(dialect=postgresql.dialect())
    )

    assert str(test_2.statement.compile(dialect=postgresql.dialect())) == str(
        db_session.query(
            getattr(model, query_arg_1), getattr(model, query_arg_2)
        ).statement.compile(dialect=postgresql.dialect())
    )
...
def test_build_option_args():
    ...
    test_1 = build_option_args(expected_value_1, accepted_args=accepted_args)
...
def test_build_query_args():
    ...
    test_1 = build_query_args(MockModel, arg_1, arg_2, accepted_args=accepted_args)
