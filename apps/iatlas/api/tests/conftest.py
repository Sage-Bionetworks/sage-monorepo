...
def data_set_id(test_db, data_set):
    ...
    (id,) = test_db.session.query(Dataset.id).filter_by(name=data_set).one_or_none()
    return id
...
def pcawg_data_set_id(test_db, pcawg_data_set):
    ...
    (id,) = (
        test_db.session.query(Dataset.id).filter_by(name=pcawg_data_set).one_or_none()
    )
    return id
...
def related_id(test_db, related):
    ...
    (id,) = test_db.session.query(Tag.id).filter_by(name=related).one_or_none()
    return id
...
def related_id3(test_db, related3):
    ...
    (id,) = test_db.session.query(Tag.id).filter_by(name=related3).one_or_none()
    return id
...
def tag_id(test_db, tag):
    ...
    (id,) = test_db.session.query(Tag.id).filter_by(name=tag).one_or_none()
    return id
...
def tag_id2(test_db, tag2):
    ...
    (id,) = test_db.session.query(Tag.id).filter_by(name=tag2).one_or_none()
    return id
...
def feature_class2_feature_names(test_db, feature_class2):
    ...
    names = (
        test_db.session.query(Feature.name)
        .filter_by(feature_class=feature_class2)
        .all()
    )
...
def hgnc_id(test_db, entrez_id):
    ...
    (hgnc_id,) = (
        test_db.session.query(Gene.hgnc_id).filter_by(entrez_id=entrez_id).one_or_none()
    )
    return hgnc_id
...
def tcga_tag_cohort_id(test_db, tcga_tag_cohort_name):
    ...
    (id,) = (
        test_db.session.query(Cohort.id)
        .filter_by(name=tcga_tag_cohort_name)
        .one_or_none()
    )
    return id
...
def pcawg_cohort_id(test_db, pcawg_cohort_name):
    ...
    (id,) = (
        test_db.session.query(Cohort.id).filter_by(name=pcawg_cohort_name).one_or_none()
    )
    return id
...
def tcga_tag_cohort_samples(client, tcga_tag_cohort_name, cohort_query):
    response = client.post(
        "/api",
        json={"query": cohort_query, "variables": {"cohort": [tcga_tag_cohort_name]}},
    )
...
def pcawg_cohort_samples(client, pcawg_cohort_name, cohort_query):
    response = client.post(
        "/api",
        json={"query": cohort_query, "variables": {"cohort": [pcawg_cohort_name]}},
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["cohorts"]
    cohort = page["items"][0]
