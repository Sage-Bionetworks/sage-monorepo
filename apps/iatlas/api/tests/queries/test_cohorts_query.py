...
def test_cohorts_cursor_pagination_last(client, common_query_builder):
    ...
    num = 5
    response = client.post(
        "/api", json={"query": query, "variables": {"paging": {"last": num}}}
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["cohorts"]
    items = page["items"]
...
def test_tag_cohort_query_by_name(
    client, common_query, tcga_tag_cohort_name, data_set, related3
):
    response = client.post(
        "/api",
        json={"query": common_query, "variables": {"cohort": [tcga_tag_cohort_name]}},
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["cohorts"]
    results = page["items"]
...
def test_tag_cohort_query_by_dataset_and_tag(
    client, common_query, tcga_tag_cohort_name, data_set, related3
):
    response = client.post(
        "/api",
        json={
            "query": common_query,
            "variables": {"dataSet": [data_set], "tag": [related3]},
        },
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["cohorts"]
...
def test_dataset_cohort_query_by_name(client, common_query, pcawg_cohort_name):
    response = client.post(
        "/api",
        json={"query": common_query, "variables": {"cohort": [pcawg_cohort_name]}},
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["cohorts"]
    results = page["items"]
...
def test_dataset_cohort_samples_query(client, samples_query, pcawg_cohort_name):
    response = client.post(
        "/api",
        json={"query": samples_query, "variables": {"cohort": [pcawg_cohort_name]}},
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["cohorts"]
    results = page["items"]
...
def test_tag_cohort_samples_query(
    client, samples_query, tcga_tag_cohort_name, data_set, related3
):
    response = client.post(
        "/api",
        json={"query": samples_query, "variables": {"cohort": [tcga_tag_cohort_name]}},
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["cohorts"]
    results = page["items"]
...
def test_tcga_cohort_samples_query(client, samples_query, data_set):
    response = client.post(
        "/api", json={"query": samples_query, "variables": {"cohort": [data_set]}}
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["cohorts"]
    results = page["items"]
...
def test_pcawg_cohort_samples_query(client, samples_query, pcawg_data_set):
    response = client.post(
        "/api", json={"query": samples_query, "variables": {"cohort": [pcawg_data_set]}}
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["cohorts"]
    results = page["items"]
...
def test_tag_cohort_features_query(
    ...
    response = client.post(
        "/api", json={"query": query, "variables": {"cohort": [tcga_tag_cohort_name]}}
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["cohorts"]
    results = page["items"]
...
def test_tag_cohort_genes_query(
    client, common_query_builder, tcga_tag_cohort_name, data_set, related3
):
    query = common_query_builder(
        """
...
def test_tag_cohort_mutations_query(
    client, common_query_builder, tcga_tag_cohort_name, data_set, related3
):
    query = common_query_builder(
        """
