...
def test_gene(test_db, test_neoantigen):
    query = test_db.session.query(Gene.id, Gene.entrez_id, Gene.hgnc_id)
...
def test_patient(test_db, test_neoantigen):
    query = test_db.session.query(Patient.id, Patient.name)
...
def test_query_specific_neoantigen(
    client, common_query, test_neoantigen, test_gene, test_patient
):
    response = client.post(
        "/api",
        json={
