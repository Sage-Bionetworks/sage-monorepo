...
def test_edges_missing_pagination(client, common_query_builder):
    ...
    response = client.post(
        ...
        json={
            "query": query,
            "variables": {"dataSet": ["TCGA"], "related": ["Immune_Subtype"]},
        },
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["edges"]
...
def test_edges_query_with_passed_maxScore_and_node2(
    client, common_query_builder, max_score, node_3
):
    query = common_query_builder(
        """{
                                    items {
    ...
    response = client.post(
        "/api",
        json={"query": query, "variables": {"maxScore": max_score, "node2": [node_3]}},
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["edges"]
    results = page["items"]
...
def test_edges_query_with_passed_minScore_and_node2(
    client, common_query_builder, min_score, node_3
):
    query = common_query_builder(
        """{
                                    items {
    ...
    response = client.post(
        "/api",
        json={"query": query, "variables": {"minScore": min_score, "node2": [node_3]}},
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["edges"]
    results = page["items"]
...
def test_edges_query_with_passed_maxScore_minScore_and_node2(
    client, common_query_builder, max_score, min_score, node_3
):
    query = common_query_builder(
        """{
                                    items {
