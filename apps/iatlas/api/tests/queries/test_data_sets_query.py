...
def test_data_sets_cursor_pagination_last(client, common_query):
    num = 1
    response = client.post(
        "/api", json={"query": common_query, "variables": {"paging": {"last": num}}}
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["dataSets"]
    items = page["items"]
...
def test_data_sets_query_with_dataSet_and_sample(
    client, samples_query, data_set, sample
):
    response = client.post(
        "/api",
        json={
            "query": samples_query,
            "variables": {"dataSet": [data_set], "sample": [sample]},
        },
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["dataSets"]
...
def test_data_sets_query_with_dataSetType(client, common_query, data_set_type):
    response = client.post(
        "/api",
        json={"query": common_query, "variables": {"dataSetType": [data_set_type]}},
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["dataSets"]
    results = page["items"]
