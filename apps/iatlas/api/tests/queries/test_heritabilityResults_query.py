...
def test_heritabilityResults_query_with_passed_data_set_and_feature(
    client, common_query, data_set, hr_feature, hr_germline_module, hr_germline_category
):
    response = client.post(
        "/api",
        json={
            "query": common_query,
            "variables": {"dataSet": [data_set], "feature": [hr_feature]},
        },
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["heritabilityResults"]
...
def test_heritabilityResults_query_with_passed_min_p_value(
    client, common_query, min_p_value
):
    response = client.post(
        "/api", json={"query": common_query, "variables": {"minPValue": min_p_value}}
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["heritabilityResults"]
    results = page["items"]
...
def test_heritabilityResults_query_with_passed_max_p_value(
    client, common_query, max_p_value
):
    response = client.post(
        "/api", json={"query": common_query, "variables": {"maxPValue": max_p_value}}
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["heritabilityResults"]
    results = page["items"]
