...
def nanostring_sample():
    return "Prat_CanRes_2017-A42-ar-A42_pre"
...
def test_cursor_pagination_first_without_samples(client, common_query_builder):
    ...
    response = client.post(
        "/api", json={"query": query, "variables": {"paging": {"first": requested_n}}}
    )
...
def test_genes_query_with_entrez(client, common_query, entrez_id, hgnc_id):
    response = client.post(
        "/api", json={"query": common_query, "variables": {"entrez": [entrez_id]}}
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["genes"]
    results = page["items"]
    ...
    for result in results:
        ...
        assert result["entrez"] == entrez_id
        assert result["hgnc"] == hgnc_id
...
def test_genes_query_with_gene_type(client, common_query, entrez_id, gene_type):
    response = client.post(
        "/api",
        json={
            "query": common_query,
            "variables": {"entrez": [entrez_id], "geneType": [gene_type]},
        },
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["genes"]
...
def test_genes_query_with_gene_type2(client, common_query, entrez_id, gene_type2):
    response = client.post(
        "/api",
        json={
            "query": common_query,
            "variables": {"entrez": [55], "geneType": [gene_type2]},
        },
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["genes"]
...
def test_genes_query_returns_publications(
    client, common_query_builder, entrez_id, hgnc_id
):
    query = common_query_builder(
        """
        {
    ...
    response = client.post(
        "/api", json={"query": query, "variables": {"entrez": [entrez_id]}}
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["genes"]
    results = page["items"]
...
def test_genes_query_returns_publications_with_geneType(
    client, common_query_builder, entrez_id, gene_type, hgnc_id
):
    query = common_query_builder(
        """
        {
...
def test_genes_rnaseq_query_with_gene_and_cohort(
    client, entrez_id, rnaseq_query, tcga_tag_cohort_name, tcga_tag_cohort_samples
):
    response = client.post(
        "/api",
        json={
            "query": rnaseq_query,
            "variables": {"entrez": [entrez_id], "cohort": [tcga_tag_cohort_name]},
        },
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["genes"]
...
def test_genes_rnaseq_query_with_gene_and_sample(
    client, entrez_id, rnaseq_query, sample
):
    response = client.post(
        "/api",
        json={
            "query": rnaseq_query,
            "variables": {"entrez": [entrez_id], "sample": [sample]},
        },
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["genes"]
...
def test_genes_query_with_entrez_and_maxRnaSeqExpr(client, rnaseq_query, entrez_id):
    max_rna_seq_expr = 1
    response = client.post(
        "/api",
        json={
            "query": rnaseq_query,
            "variables": {"maxRnaSeqExpr": max_rna_seq_expr, "entrez": entrez_id},
        },
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["genes"]
...
def test_genes_query_with_entrez_and_minRnaSeqExpr(client, rnaseq_query, entrez_id):
    min_rna_seq_expr = 1
    response = client.post(
        "/api",
        json={
            "query": rnaseq_query,
            "variables": {"minRnaSeqExpr": min_rna_seq_expr, "entrez": entrez_id},
        },
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["genes"]
...
def test_genes_nanostring_query_with_gene_and_sample(
    client, nanostring_query, nanostring_entrez_id, nanostring_sample
):
    response = client.post(
        "/api",
        json={
    ...
    assert len(genes) == 1
    gene = genes[0]
    assert gene["entrez"] == nanostring_entrez_id
    samples = gene["samples"]
    assert isinstance(samples, list)
    assert len(samples) == 1
    s = samples[0]
...
def test_pseudobulk_query_with_entrez(client, pseudobulk_query):
    response = client.post(
        "/api", json={"query": pseudobulk_query, "variables": {"entrez": [135]}}
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["genes"]
    genes = page["items"]
...
def test_cells_query_with_entrez(client, cells_query):
    response = client.post(
        "/api", json={"query": cells_query, "variables": {"entrez": [135]}}
    )
    json_data = json.loads(response.data)
    page = json_data["data"]["genes"]
    genes = page["items"]
    ...
    for cell in cells[0:10]:
        ...
        assert isinstance(cell["singleCellSeq"], float)
