...
def test_mutations_cursor_pagination_first_without_samples(
...
def test_mutations_query_with_mutationCode(client, common_query, mutation_code):
    response = client.post(
        "/api",
        json={"query": common_query, "variables": {"mutationCode": [mutation_code]}},
    )
    json_data = json.loads(response.data)
    mutations = json_data["data"]["mutations"]
    page = mutations["items"]
...
def test_mutations_query_with_mutationType(client, common_query, mutation_type_name):
    response = client.post(
        ...
        json={
            "query": common_query,
            "variables": {"mutationType": [mutation_type_name]},
        },
    )
    json_data = json.loads(response.data)
    mutations = json_data["data"]["mutations"]
...
def test_mutations_query_with_status(client, samples_query, mutation_status):
    ...
    response = client.post(
        ...
        json={
            "query": samples_query,
            "variables": {"paging": {"first": num}, "status": [mutation_status]},
        },
    )
    json_data = json.loads(response.data)
    mutations = json_data["data"]["mutations"]
...
def test_mutations_query_with_cohort(
    client, samples_query, tcga_tag_cohort_name, tcga_tag_cohort_samples
):
    num = 1
    response = client.post(
        "/api",
        json={
            "query": samples_query,
            "variables": {"paging": {"first": num}, "cohort": [tcga_tag_cohort_name]},
        },
    )
    json_data = json.loads(response.data)
    mutations = json_data["data"]["mutations"]
