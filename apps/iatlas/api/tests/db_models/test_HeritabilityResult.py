...
def hr_feature_id(test_db, hr_feature):
    ...
    (id,) = test_db.session.query(Feature.id).filter_by(name=hr_feature).one_or_none()
    return id


def test_HeritabilityResult_with_relations(
    app, data_set, data_set_id, hr_feature, hr_feature_id
):
    ...
    results = (
        ...
        .filter_by(feature_id=hr_feature_id)
        .limit(3)
        .all()
    )
