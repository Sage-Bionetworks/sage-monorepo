...
def gs_gene_id(test_db, gs_entrez_id):
    ...
    (id,) = (
        test_db.session.query(Gene.id).filter_by(entrez_id=gs_entrez_id).one_or_none()
    )
    return id
...
def nanostring_sample_id(test_db, nanostring_sample):
    ...
    (id,) = (
        test_db.session.query(Sample.id).filter_by(name=nanostring_sample).one_or_none()
    )
    return id
...
def nanostring_gene_id(test_db, nanostring_entrez_id):
    ...
    (id,) = (
        ...
        .filter_by(entrez_id=nanostring_entrez_id)
        .one_or_none()
    )
    return id
...
def test_GeneToSample_nanostring(app, nanostring_sample_id, nanostring_gene_id):
    query = return_gene_to_sample_query()
    results = (
        query.filter_by(sample_id=nanostring_sample_id)
        .filter_by(gene_id=nanostring_gene_id)
        .all()
    )
