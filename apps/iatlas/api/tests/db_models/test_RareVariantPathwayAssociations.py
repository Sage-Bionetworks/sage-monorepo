...
def rvap_feature_id(test_db, rvap_feature):
    ...
    (id,) = test_db.session.query(Feature.id).filter_by(name=rvap_feature).one_or_none()
    return id
...
def test_RareVariantPathwayAssociation_with_relations(
    ...
    results = (
        ...
        .filter_by(feature_id=rvap_feature_id)
        .filter_by(pathway=rvap_pathway)
        .limit(3)
        .all()
    )
    ...
    for result in results:
        ...
        string_representation = (
            "<RareVariantPathwayAssociation %r>" % rare_variant_pathway_association_id
        )
...
def test_RareVariantPathwayAssociation_no_relations(
    app, data_set_id, rvap_feature_id, rvap_pathway
):
