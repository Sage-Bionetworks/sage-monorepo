...
def publication_id_with_tag(test_db, publication_title_with_tag):
    ...
    (id,) = (
        test_db.session.query(Publication.id)
        .filter_by(title=publication_title_with_tag)
        .one_or_none()
    )
    return id


def test_TagToPublication_with_relations(
    app, publication_title_with_tag, publication_id_with_tag
):
