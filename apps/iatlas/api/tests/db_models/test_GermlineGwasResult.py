...
def ggr_feature_id(test_db, ggr_feature):
    ...
    (id,) = test_db.session.query(Feature.id).filter_by(name=ggr_feature).one_or_none()
    return id
...
def ggr_snp_id(test_db, ggr_snp):
    ...
    (id,) = test_db.session.query(Snp.id).filter_by(name=ggr_snp).one_or_none()
    return id


def test_GermlineGwasResult_with_relations(
    app, data_set, data_set_id, ggr_feature, ggr_feature_id, ggr_snp, ggr_snp_id
):
    ...
    results = (
        ...
        .filter_by(feature_id=ggr_feature_id)
        .filter_by(snp_id=ggr_snp_id)
        .limit(3)
        .all()
    )
