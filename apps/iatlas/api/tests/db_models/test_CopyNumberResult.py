...
def feature_id(test_db, cnr_feature):
    ...
    (id,) = test_db.session.query(Feature.id).filter_by(name=cnr_feature).one_or_none()
    return id
...
def cnr_tag_id(test_db, cnr_tag):
    ...
    (id,) = test_db.session.query(Tag.id).filter_by(name=cnr_tag).one_or_none()
    return id


def test_CopyNumberResult_with_relations(
    app, data_set_id, entrez_id, gene_id, cnr_tag, cnr_tag_id
):
