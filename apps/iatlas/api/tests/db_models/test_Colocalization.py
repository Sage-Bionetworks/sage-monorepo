...
def coloc_feature_id(test_db, coloc_feature):
    ...
    (id,) = (
        test_db.session.query(Feature.id).filter_by(name=coloc_feature).one_or_none()
    )
    return id
...
def coloc_snp_id(test_db, coloc_snp_name):
    ...
    (id,) = test_db.session.query(Snp.id).filter_by(name=coloc_snp_name).one_or_none()
    return id
...
def coloc_gene_id(test_db, coloc_gene_entrez_id):
    ...
    (id,) = (
        test_db.session.query(Gene.id)
        .filter_by(entrez_id=coloc_gene_entrez_id)
        .one_or_none()
    )
    return id
...
def coloc_data_set1_id(test_db, coloc_data_set1):
    ...
    (id,) = (
        test_db.session.query(Dataset.id).filter_by(name=coloc_data_set1).one_or_none()
    )
    return id
...
def coloc_data_set2_id(test_db, coloc_data_set2):
    ...
    (id,) = (
        test_db.session.query(Dataset.id).filter_by(name=coloc_data_set2).one_or_none()
    )
    return id
...
def test_sQTL_Colocalization_with_relations(
    app,
    data_set,
    data_set_id,
    coloc_data_set1,
    ...
    separator = ", "
    relationships_to_join = ["data_set", "coloc_data_set", "feature", "snp", "gene"]

    query = return_colocalization_query(*relationships_to_join)
    results = (
        query.filter_by(dataset_id=data_set_id)
        .filter_by(coloc_dataset_id=coloc_data_set1_id)
        .filter_by(feature_id=coloc_feature_id)
        .filter_by(snp_id=coloc_snp_id)
        .filter_by(gene_id=coloc_gene_id)
        .filter_by(qtl_type=coloc_qtl_type1)
        .filter_by(ecaviar_pp=coloc_ecaviar_pp)
        .filter_by(plot_type=coloc_plot_type1)
        .filter_by(splice_loc=coloc_splice_loc)
        .all()
    )
...
def test_eQTL_Colocalization_with_relations(
    ...
    results = (
        query.filter_by(dataset_id=data_set_id)
        .filter_by(coloc_dataset_id=coloc_data_set2_id)
        .filter_by(feature_id=coloc_feature_id)
        .filter_by(snp_id=coloc_snp_id)
        .filter_by(gene_id=coloc_gene_id)
        .filter_by(qtl_type=coloc_qtl_type2)
        .filter_by(plot_type=coloc_plot_type2)
        .filter_by(tissue=coloc_tissue)
        .all()
    )
...
def test_Colocalization_no_relations(
    app, data_set_id, coloc_feature_id, coloc_gene_id, coloc_snp_id
):
    query = return_colocalization_query()
    results = (
        query.filter_by(dataset_id=data_set_id)
        .filter_by(coloc_dataset_id=data_set_id)
        .filter_by(feature_id=coloc_feature_id)
        .filter_by(snp_id=coloc_snp_id)
        .filter_by(gene_id=coloc_gene_id)
        .limit(3)
