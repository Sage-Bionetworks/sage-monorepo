...
def resolve_mutations(_obj, info, cohort=None, distinct=False, entrez=None, mutationCode=None, mutationId=None, mutationType=None, paging=None, sample=None, status=None):
    ...
    if len(sample_requested):
        ...
        sample_dict = return_mutation_derived_fields(
            requested, patient_requested, sample_requested, cohort=cohort, entrez=entrez, mutation_id=mutation_ids, mutation_code=mutationCode, mutation_type=mutationType, sample=sample, status=status)
    else:
        items = fetch_page(query, paging, distinct)
