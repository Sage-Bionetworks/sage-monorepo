...
def resolve_cohorts(_obj, info, distinct=False, paging=None, name=None, dataSet=None, tag=None, clinical=None):
    ...
    samples = get_cohort_samples(
        requested, sample_requested, sample_tag_requested, name=name, data_set=dataSet, tag=tag, clinical=clinical)

    features = get_cohort_features(
        requested, feature_requested, name=name, data_set=dataSet, tag=tag, clinical=clinical)

    genes = get_cohort_genes(
        requested, gene_requested, name=name, data_set=dataSet, tag=tag, clinical=clinical)
