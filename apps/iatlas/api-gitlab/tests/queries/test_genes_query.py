...
def test_genes_query_with_gene_type(client, common_query, entrez_id, gene_type):
    response = client.post(
        '/api', json={'query': common_query, 'variables': {'entrez': [entrez_id], 'geneType': [gene_type]}})
    json_data = json.loads(response.data)
    page = json_data['data']['genes']
    results = page['items']
...
def test_genes_query_returns_publications(client, common_query_builder, entrez_id, hgnc_id):
    query = common_query_builder(
        """
        {
            items{
...
def test_genes_query_returns_publications_with_geneType(client, common_query_builder, entrez_id, gene_type, hgnc_id):
    query = common_query_builder(
        """
        {
            items{
    ...
    response = client.post(
        '/api', json={'query': query, 'variables': {'entrez': [entrez_id], 'geneType': [gene_type]}})
    json_data = json.loads(response.data)
    page = json_data['data']['genes']
    results = page['items']
...
def test_genes_rnaseq_query_with_gene_and_cohort(client, entrez_id, rnaseq_query, tcga_tag_cohort_name, tcga_tag_cohort_samples):
    response = client.post(
        '/api', json={
            'query': rnaseq_query,
            'variables': {
...
def test_genes_rnaseq_query_with_gene_and_sample(client, entrez_id, rnaseq_query, sample):
    response = client.post(
        '/api', json={
            'query': rnaseq_query,
            'variables': {
