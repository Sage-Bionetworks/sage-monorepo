...
class TranscriptomicsApiControllerWebTest {

  private TranscriptomicsApiDelegate delegate;
  private MockMvc mockMvc;

  @BeforeEach
  void setUp() {
    delegate = mock(TranscriptomicsApiDelegate.class);
    var controller = new TranscriptomicsApiController(delegate);
    ...
    conversionService.addConverter(
      new Converter<String, TranscriptomicsSearchQueryDto.SortOrdersEnum>() {
        @Override
  ...
  void shouldReturnBadRequestProblemWhenDelegateRaisesInvalidObjectIdException() throws Exception {
    when(delegate.getTranscriptomics(any())).thenThrow(new InvalidObjectIdException("not-an-id"));
  ...
  @Test
  @DisplayName("should return not found problem when transcriptomics not found")
  void shouldReturnNotFoundProblemWhenTranscriptomicsNotFound() throws Exception {
    String transcriptomicsId = "673f5d8e8c1a2b3c4d5e6f7a";
    when(delegate.getTranscriptomics(any())).thenThrow(
      new ResponseStatusException(
        HttpStatus.NOT_FOUND,
        "Transcriptomics not found with id: " + transcriptomicsId
      )
    );
