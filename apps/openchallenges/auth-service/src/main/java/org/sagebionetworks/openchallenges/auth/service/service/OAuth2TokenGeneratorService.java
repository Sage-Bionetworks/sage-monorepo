...
public class OAuth2TokenGeneratorService {
  ...
  public String generateRefreshToken(User user) {
    ...
    log.debug(
      "Built refresh token context with grant type: {}",
    ...
    OAuth2Token token = tokenGenerator.generate(tokenContext);
    log.debug(
      "Refresh token generator returned: {}",
      token != null ? token.getClass().getSimpleName() : "null"
    );
    ...
    if (token instanceof OAuth2RefreshToken) {
      ...
      throw new RuntimeException(
        "Unexpected refresh token type: " + token.getClass().getSimpleName()
      );
    }
