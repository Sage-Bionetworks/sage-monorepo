...
public class UserService {
  ...
  public Optional<User> authenticate(String username, String password) {
    ...
    if (userOpt.isPresent()) {
      ...
      log.debug("Authentication failed: user not found or disabled: {}", username);
    }
  ...
  public void setUserEnabled(User user, boolean enabled) {
    log.info("Setting user {} enabled status to: {}", user.getUsername(), enabled);
    user.setEnabled(enabled);
    userRepository.save(user);
    log.info("Successfully updated enabled status for user: {}", user.getUsername());
  }
}
