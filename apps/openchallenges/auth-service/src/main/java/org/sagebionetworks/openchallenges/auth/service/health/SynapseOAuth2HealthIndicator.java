...
public class SynapseOAuth2HealthIndicator extends AbstractHealthIndicator {
  ...
  // Synapse OAuth2 discovery endpoint - well-known configuration URL
  ...
  /**
   * Performs the actual health check for Synapse OAuth2 provider.
   *
  ...
  protected void doHealthCheck(Health.Builder builder) {
    try {
      ...
      if (cached != null && !cached.isExpired()) {
        log.debug("Using cached Synapse OAuth2 health result");
      ...
      log.debug("Performing fresh Synapse OAuth2 health check");
      ...
      log.error("Unexpected error during Synapse OAuth2 health check", e);
      builder
        .down()
  ...
  /**
   * Performs the actual HTTP request to check Synapse OAuth2 discovery endpoint.
   *
  ...
  private Health performHealthCheck() {
    ...
    try {
      log.debug("Checking Synapse OAuth2 discovery endpoint: {}", SYNAPSE_DISCOVERY_URL);

      @SuppressWarnings("rawtypes")
      ResponseEntity<Map> response = restTemplate.getForEntity(SYNAPSE_DISCOVERY_URL, Map.class);
      ...
      if (statusCode.is2xxSuccessful() && body != null) {
        ...
        boolean hasRequiredFields =
          body.containsKey("authorization_endpoint") &&
          body.containsKey("token_endpoint") &&
          body.containsKey("issuer");

        if (hasRequiredFields) {
          log.debug(
            "Synapse OAuth2 health check successful - response time: {}ms",
            responseTime.toMillis()
          );

          return Health.up()
            .withDetail("provider", "Synapse OAuth2")
            .withDetail("discoveryUrl", SYNAPSE_DISCOVERY_URL)
          ...
          log.warn("Synapse OAuth2 discovery response missing required fields");
          return Health.down()
            .withDetail("provider", "Synapse OAuth2")
            .withDetail("discoveryUrl", SYNAPSE_DISCOVERY_URL)
        ...
        log.warn("Synapse OAuth2 discovery endpoint returned non-success status: {}", statusCode);
        return Health.down()
          .withDetail("provider", "Synapse OAuth2")
          .withDetail("discoveryUrl", SYNAPSE_DISCOVERY_URL)
      ...
      Duration responseTime = Duration.between(startTime, Instant.now());
      log.warn("Synapse OAuth2 discovery endpoint unreachable", e);
