...
def main() -> None:
    ...
    )  # Different from dev to avoid overlap
    certificate_arn = os.getenv("CERTIFICATE_ARN", "")
    max_azs = int(os.getenv("MAX_AZS", "2"))  # Number of Availability Zones
    app_version = os.getenv("APP_VERSION", "latest")  # Use stable releases for stage
    ...
    placeholder_enabled = os.getenv(
        "IMAGE_SERVICE_PLACEHOLDER_ENABLED", "false"
    )  # Stage should use real images
    ...
    # Stage uses production-like Multi-AZ deployment for high availability testing
    database_stack = DatabaseStack(
        app,
        f"{stack_prefix}-database",
        stack_prefix=stack_prefix,
        environment=environment,
        developer_name=None,  # Stage is shared, no developer isolation
        vpc=vpc_stack.vpc,
