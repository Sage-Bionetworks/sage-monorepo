# coding: utf-8

from __future__ import absolute_import
from datetime import date, datetime  # noqa: F401

from typing import List, Dict  # noqa: F401

from openapi_server.models.base_model_ import Model
from openapi_server.models.challenge_organizer_create_request import ChallengeOrganizerCreateRequest
from openapi_server.models.challenge_organizer_create_response import ChallengeOrganizerCreateResponse
from openapi_server.models.challenge_organizer_role import ChallengeOrganizerRole
import re
from openapi_server import util

from openapi_server.models.challenge_organizer_create_request import ChallengeOrganizerCreateRequest  # noqa: E501
from openapi_server.models.challenge_organizer_create_response import ChallengeOrganizerCreateResponse  # noqa: E501
from openapi_server.models.challenge_organizer_role import ChallengeOrganizerRole  # noqa: E501
import re  # noqa: E501

class ChallengeOrganizer(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, id=None, name=None, login=None, roles=None):  # noqa: E501
        """ChallengeOrganizer - a model defined in OpenAPI

        :param id: The id of this ChallengeOrganizer.  # noqa: E501
        :type id: str
        :param name: The name of this ChallengeOrganizer.  # noqa: E501
        :type name: str
        :param login: The login of this ChallengeOrganizer.  # noqa: E501
        :type login: str
        :param roles: The roles of this ChallengeOrganizer.  # noqa: E501
        :type roles: List[ChallengeOrganizerRole]
        """
        self.openapi_types = {
            'id': str,
            'name': str,
            'login': str,
            'roles': List[ChallengeOrganizerRole]
        }

        self.attribute_map = {
            'id': 'id',
            'name': 'name',
            'login': 'login',
            'roles': 'roles'
        }

        self._id = id
        self._name = name
        self._login = login
        self._roles = roles

    @classmethod
    def from_dict(cls, dikt) -> 'ChallengeOrganizer':
        """Returns the dict as a model

        :param dikt: A dict.
        :type: dict
        :return: The ChallengeOrganizer of this ChallengeOrganizer.  # noqa: E501
        :rtype: ChallengeOrganizer
        """
        return util.deserialize_model(dikt, cls)

    @property
    def id(self):
        """Gets the id of this ChallengeOrganizer.

        The unique identifier of a challenge organizer  # noqa: E501

        :return: The id of this ChallengeOrganizer.
        :rtype: str
        """
        return self._id

    @id.setter
    def id(self, id):
        """Sets the id of this ChallengeOrganizer.

        The unique identifier of a challenge organizer  # noqa: E501

        :param id: The id of this ChallengeOrganizer.
        :type id: str
        """
        if id is None:
            raise ValueError("Invalid value for `id`, must not be `None`")  # noqa: E501

        self._id = id

    @property
    def name(self):
        """Gets the name of this ChallengeOrganizer.


        :return: The name of this ChallengeOrganizer.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name):
        """Sets the name of this ChallengeOrganizer.


        :param name: The name of this ChallengeOrganizer.
        :type name: str
        """
        if name is None:
            raise ValueError("Invalid value for `name`, must not be `None`")  # noqa: E501

        self._name = name

    @property
    def login(self):
        """Gets the login of this ChallengeOrganizer.

        The user or organization account name  # noqa: E501

        :return: The login of this ChallengeOrganizer.
        :rtype: str
        """
        return self._login

    @login.setter
    def login(self, login):
        """Sets the login of this ChallengeOrganizer.

        The user or organization account name  # noqa: E501

        :param login: The login of this ChallengeOrganizer.
        :type login: str
        """
        if login is not None and len(login) > 25:
            raise ValueError("Invalid value for `login`, length must be less than or equal to `25`")  # noqa: E501
        if login is not None and len(login) < 3:
            raise ValueError("Invalid value for `login`, length must be greater than or equal to `3`")  # noqa: E501
        if login is not None and not re.search(r'^[a-z0-9]+(?:-[a-z0-9]+)*$', login):  # noqa: E501
            raise ValueError("Invalid value for `login`, must be a follow pattern or equal to `/^[a-z0-9]+(?:-[a-z0-9]+)*$/`")  # noqa: E501

        self._login = login

    @property
    def roles(self):
        """Gets the roles of this ChallengeOrganizer.


        :return: The roles of this ChallengeOrganizer.
        :rtype: List[ChallengeOrganizerRole]
        """
        return self._roles

    @roles.setter
    def roles(self, roles):
        """Sets the roles of this ChallengeOrganizer.


        :param roles: The roles of this ChallengeOrganizer.
        :type roles: List[ChallengeOrganizerRole]
        """

        self._roles = roles
