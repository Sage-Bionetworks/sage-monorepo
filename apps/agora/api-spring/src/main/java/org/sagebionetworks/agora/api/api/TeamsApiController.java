...
public class TeamsApiController implements TeamsApi {
    ...
    private final TeamsApiDelegate delegate;

    public TeamsApiController(@Autowired(required = false) TeamsApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new TeamsApiDelegate() {});
    }

    @Override
    public TeamsApiDelegate getDelegate() {
        return delegate;
    }
