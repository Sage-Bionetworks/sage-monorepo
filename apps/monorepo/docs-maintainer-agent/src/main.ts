...
class DocsMaintenanceAgent {
  ...
  private async generateApiDocs(spec: OpenAPISpec): Promise<DocumentationUpdate | null> {
    try {
      ...
      if (parsedSpec.components?.securitySchemes) {
        ...
        Object.entries(parsedSpec.components.securitySchemes).forEach(([name, scheme]: [string, any]) => {
    ...
    try {
      ...
      for (const projectPath of projectPaths) {
        try {
          ...
          const language = 
            tags.find((tag: string) => tag.startsWith('language:'))?.replace('language:', '') || 'unknown';
          ...
          if (scopeProjects) {
            scopeProjects.push({
              ...
              hasReadme,
              description: this.extractProjectDescription(projectConfig, path.join(this.config.repoPath, readmePath)),
            });
          }
        } catch (error) {
      ...
      sortedScopes.forEach((scope) => {
        ...
        if (apps.length > 0) {
          ...
          apps.sort((a, b) => a.name.localeCompare(b.name)).forEach((project) => {
        ...
        if (libs.length > 0) {
          content += `### Libraries\n\n`;
          libs.sort((a, b) => a.name.localeCompare(b.name)).forEach((project) => {
            content += `#### ${project.name}\n\n`;
            if (project.description) {
              content += `${project.description}\n\n`;
            }
    ...
    try {
      ...
      for (const projectPath of projectPaths) {
        try {
          ...
          const scopeTag = tags.find((tag: string) => tag.startsWith('scope:'))?.split(':')[1] || 'unknown';
          const languageTag = tags.find((tag: string) => tag.startsWith('language:'))?.split(':')[1] || 'unknown';
          
          // Determine project type
          const projectType = projectConfig.projectType === 'application' ? 'application' : 'library';
  ...
  private generateProjectTable(projects: ProjectInfo[]): string {
    ...
    const projectsByScope = projects.reduce((acc, project) => {
    ...
    Object.keys(projectsByScope).sort().forEach(scope => {
